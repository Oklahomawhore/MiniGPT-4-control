...
def parse_args():
    ...
    parser.add_argument('--negative-sample', action='store_true')
...
def ask_llm(chat,image,patch, prompt, args=None, CONV_VISION=None):
...
def save_resuls(output_dir, result, suffix):
...
def main():
    ...
    chat, CONV_VISION, cfg = init_model()
    ASR = 0.0
    hit_count_clean = 0
    hit_count_patch = 0
    total_count = 0
    ...
    output_dir = f"{args.output_dir}/{experiment_id}"
    ...
    for img in pbar:
        ...
        llm_message_clean = ask_llm(chat,image, False, prompt, CONV_VISION=CONV_VISION, args=args)
        llm_message_patch = ask_llm(chat, image, True, prompt, CONV_VISION=CONV_VISION, args=args)
        if copy_debug:
    ...
    resFile_clean = save_resuls(output_dir, result_clean, "clean_short")
    resFile_patch = save_resuls(output_dir, result_patch, "patch_short")
    _ = save_resuls(output_dir, succ_img_clean, "clean_normal")
    _ = save_resuls(output_dir, succ_img_patch, "patch_normal")
